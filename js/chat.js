...
document.addEventListener('DOMContentLoaded', function() {
  ...
  try {
    ...
    n8nChatClient = window.createChat({
      webhookUrl: 'https://workwithborj.app.n8n.cloud/webhook/854c829c-2ce6-426f-89e2-ed44d33182f3/chat',
      webhookConfig: {
        method: 'POST',
        headers: {}
      },
  ...
  function initializeCustomChat() {
    ...
    if (savedMessages.length > 0) {
      ...
      addMessage('Hi there! My name is BorjBot. I am an AI Chatbot for WorkWithBorj.', 'bot');
    ...
    async function sendMessage() {
      ...
      try {
            ...
            response.forEach(msg => {
              if (typeof msg === 'string') {
                try {
                  ...
                  const msgText = jsonMsg.output || jsonMsg.text || jsonMsg.response || msg;
                  addMessage(msgText, 'bot');
                } catch (e) {
                  addMessage(msg, 'bot');
                }
              } else {
                const msgText = msg.output || msg.text || msg.response || JSON.stringify(msg);
                addMessage(msgText, 'bot');
              }
            });
          } 
        ...
        try {
          ...
          const fallbackResponse = await fetch('https://workwithborj.app.n8n.cloud/webhook/854c829c-2ce6-426f-89e2-ed44d33182f3/chat', {
            method: 'POST',
            headers: {
